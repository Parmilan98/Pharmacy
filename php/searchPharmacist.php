...
<html lang="en">
  ...
  <div class="sidebar close">
    ...
    <section class="home-section">
      ...
      <div class="p-2">
        <main class="px-4">
          <h4 class="text-center py-3">Search Pharmacist</h4>
          
          <form action="searchPharmacist.php" method="post">
                <div class="row">
          ...
          {
            $sql = "SELECT * FROM pharmacist WHERE PHANAME like '%{$_POST["search"]}%' or PHANUMBER like '%{$_POST["search"]}%' or PHAEMAIL like '%{$_POST["search"]}%'";
            $result = $db->query($sql);
            if($result->num_rows>0)
            {
              echo "<table class='table table-striped table-hover'>
                      <tr>
                        <th class='text-light'>SNO</th>
                      ...
                      { 
                        ...
                        echo "<td>{$row["PHANUMBER"]}</td>";
                        echo "<td>{$row["PHAEMAIL"]}</td>";
                        echo "<td>{$row["PHAADDRESS"]}</td>";
                       
                        echo "</tr>";
                      }
              echo "</table>";
            }else{
              echo "<p class='alert alert-danger fw-bold w-50'>No searching Supplier Rocords Found.</p>";
            }
        }
        ?>
        <h4 class="text-center py-4">View all supplier Details</h4>
        <?php
